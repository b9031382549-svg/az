...
use App\Jobs\GeneratePrecedentEmbeddings;
use App\Services\Embeddings\PrecedentEmbeddingRunner;
use Illuminate\Console\Command;

class EmbedPrecedents extends Command
{
    protected $signature = 'data:embed-precedents
    ...
    protected $description = 'Generate bge-m3 embeddings for precedent rows (resumable; --queue runs it in the background)';

    public function handle(PrecedentEmbeddingRunner $runner): int
    {
        ...
        if ($pending === 0) {
            $this->info('Nothing to embed — all precedent rows already have embeddings.');

            return self::SUCCESS;
        }

        if ($this->option('queue')) {
            GeneratePrecedentEmbeddings::dispatch($chunk);
            $this->info("Dispatched embedding of {$pending} rows to the queue (processed by the worker).");

            return self::SUCCESS;
        }

        $this->info("Embedding {$pending} precedent rows (batch size {$chunk}) ...");
    ...
    private function refresh(PrecedentEmbeddingRunner $runner, int $chunk): int
    {
        $before = now()->toDateTimeString();

        if ($this->option('queue')) {
            GeneratePrecedentEmbeddings::dispatch($chunk, $before);
            $this->info('Dispatched an in-place re-embed of all precedent rows to the queue.');

            return self::SUCCESS;
        }

        $total = $runner->staleCount($before);
        $this->info("Re-embedding {$total} precedent rows in place ...");
