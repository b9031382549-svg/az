...
<div class="min-h-screen flex">
  ...
  <aside class="hidden md:flex flex-col w-60 shrink-0 bg-paper border-r hair">
    ...
    <nav class="flex-1 py-4">
      ...
      <div class="px-5 pt-4 pb-1.5"><p class="kicker">{{ __('Classifier') }}</p></div>
      ...
      <a href="{{ route('benchmark') }}" class="{{ $nav('benchmark') }}">
        <svg fill="none" stroke="currentColor" stroke-width="1.6" viewBox="0 0 20 20" stroke-linecap="round" stroke-linejoin="round"><path d="M3 17h14"/><rect x="4" y="9" width="3" height="8"/><rect x="8.5" y="5" width="3" height="12"/><rect x="13" y="11" width="3" height="6"/></svg>{{ __('Benchmark') }}</a>
  ...
  <div class="flex-1 flex flex-col min-w-0">
    ...
    <header class="h-16 shrink-0 flex items-center gap-4 px-5 sm:px-7 border-b hair bg-paper/80 backdrop-blur sticky top-0 z-30">
      ...
      <form action="{{ route('invoices') }}" method="GET" class="flex-1 max-w-md flex items-center gap-2 bg-surface border hair rounded-xl px-3.5 h-10">
        ...
        <input name="q" value="{{ request('q') }}" placeholder="{{ __('Search invoices, TIN, number…') }}" class="w-full bg-transparent outline-none text-sm">
      </form>
      <div class="ml-auto flex items-center gap-3">
        ...
        <form method="POST" action="{{ route('locale.set') }}">
          ...
          <select name="locale" onchange="this.form.submit()"
                  class="h-10 px-2.5 rounded-xl border hair bg-surface text-sm hover:border-ink transition outline-none cursor-pointer"
                  aria-label="{{ __('Language') }}">
            <option value="en" @selected(app()->getLocale()==='en')>EN</option>
            <option value="az" @selected(app()->getLocale()==='az')>AZ</option>
            <option value="ru" @selected(app()->getLocale()==='ru')>RU</option>
          </select>
        </form>
        <button class="w-10 h-10 grid place-items-center border hair rounded-xl bg-surface hover:border-ink transition" title="{{ __('Notifications') }}">◔</button>
        <form method="POST" action="{{ route('logout') }}">
          ...
          <button type="submit" class="flex items-center gap-2 border hair rounded-xl bg-surface px-2.5 h-10 hover:border-ink transition">
            ...
            <span class="text-sm hidden sm:inline">{{ __('Sign out') }}</span>
          </button>
        </form>
      </div>
    </header>
